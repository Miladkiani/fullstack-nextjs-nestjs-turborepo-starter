...
- If a field changes in the backend validation, the frontend build fails until the form is updated.
- A custom `check-schema-sync.js` script ensures the GraphQL schema and frontend types never diverge.

### 2. Optimized Data Fetching (Dual-Client Pattern)

Most Next.js apps suffer from unnecessary network overhead during SSR. This architecture implements a dual-client strategy:
...
- **SEO & Discoverability:** Apps leverage the Next.js Metadata API to generate dynamic OpenGraph tags, JSON-LD schema, and sitemaps for optimal search engine ranking.
...
- **Security & SEO:** Implements **Content Security Policy (CSP)** for robust defense and a complete SEO suite (OpenGraph, Sitemap, JSON-LD schema).
...
- **Focus:** Distributed architecture, and GraphQL API delivery.
...
The monorepo architecture centers around a high-performance, shared ecosystem. This ensures that the **Website** and **Admin** dashboard maintain a unified look and feel while sharing 100% of the business logic.
